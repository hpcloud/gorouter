...
func (r *AccessLogRecord) makeRecord() string {
	...
	return fmt.Sprintf(`%s - [%s] "%s %s %s" %s %d %d "%s" "%s" %s x_forwarded_for:"%s" x_forwarded_proto:"%s" vcap_request_id:%s response_time:%s app_id:%s%s endpoint:%s`+"\n",
		r.Request.Host,
		r.FormatStartedAt(),
		r.Request.Method,
